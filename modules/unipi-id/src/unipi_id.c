...
static const struct unipi_id_family_data unipi_id_family_ids[] = {
	...
	{ UNIEE_PLATFORM_ID_IRISX7,     "Iris", {0x50,0x51,0x52,0x53,0x54,0x55,0x56},
	                                        {0x48,0x49,0x4a,0x4b,0x4c,0x4d,0x4e},
	                                        irisx7_iogroups},
	{ UNIEE_PLATFORM_ID_IRISX71,     "Iris71", {0x50,0x51,0x52,0x53,0x54,0x55,0x56},
	                                        {0x48,0x49,0x4a,0x4b,0x4c,0x4d,0x4e},
	                                        irisx71_iogroups},
...
static int do_checksum(struct device *dev,struct unipi_id_data *unipi_id, unsigned char *digest)
{
	...
	for (i=0; i<unipi_id->slot_count; i++) {
		...
		if (size>0) {
			ret = crypto_shash_update(sdesc, data, size);
			if(ret!=0) return ret;
			//memcpy(data, nvmem_index, sizeof(nvmem_index));
			//ret = crypto_shash_update(sdesc, data, size);
			ret = crypto_shash_update(sdesc, (const u8 *)&nvmem_index, sizeof(nvmem_index));
			if(ret!=0) return ret;
		}
	}
